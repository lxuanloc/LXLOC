...
<!------------ Content wrapper starts -------------->
  <div class="content_wrapper">
   ...
   $run_cart = mysqli_query($con, "select * from song where ip_address='$ip' ");
   
   while($fetch_cart = mysqli_fetch_array($run_cart)){
       
	   $product_id = $fetch_cart['product_id'];
	   
	   $result_product = mysqli_query($con, "select * from song where product_id = '$product_id'");
	   
       while($fetch_product = mysqli_fetch_array($result_product)){
                
		$product_price = array($fetch_product['song_price']);

        $product_title = $fetch_product['song_title'];

        $product_image = $fetch_product['song_image'];
        
        $sing_price = $fetch_product['song_price'];
        
		$values = array_sum($song_price);
		
		// Getting Quality of the product
		
		$run_qty = mysqli_query($con, "select * from cart where song_id = '$product_id'");
